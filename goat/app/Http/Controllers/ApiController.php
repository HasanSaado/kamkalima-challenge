...
{
    public function newsapi(Request $request,$name)
    {
      ...
      $data['news']         = $api->getNews($source,$name); // Passed  source id to our api model, to fetch news by the selected source
      $data['news_sources'] = $this->allSources(); //retrieve all news sources

      return view('team_news', $data);
    }
